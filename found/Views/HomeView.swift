...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onChange(of: demoMode) { _, enabled in
    ...
    private func prepareData() async {
        do {
            ...
            if UserDefaults.standard.bool(forKey: "lostandfound.demo-mode") { try reports.seedDemoIfNeeded() }
    ...
    private var headerSection: some View { VStack(alignment: .leading, spacing: 6) {
        Text("Lost & Found").font(.system(size: 38, weight: .bold, design: .rounded))
