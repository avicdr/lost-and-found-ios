...
final class ReportRepository {
    ...
    func seedDemoIfNeeded() throws {
        let hasSeed = try context.fetchCount(FetchDescriptor<ItemReport>(predicate: #Predicate { $0.isDemo })) > 0
        guard !hasSeed else { return }
        for seed in DemoData.reports {
            context.insert(ItemReport(id: seed.id, ownerID: seed.ownerID, reportType: seed.reportType, name: seed.name,
